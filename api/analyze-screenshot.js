...
- ベージュ：黄み・オレンジみを含んだ、明るめのライトブラウン〜ベージュ系。日本人の地毛より明らかに明るく、抜け感のある柔らかい色味。
【判定のコツ】
1. まず「暗い（黒髪・暗髪ブラウンの候補）」か「明るい（アッシュグレー・ベージュの候補）」かを判断する。
2. 暗い場合：赤み・茶色みが全く無ければ「黒髪」、うっすら茶色みが見えれば「暗髪ブラウン」。
3. 明るい場合：グレー・くすみがかって寒色系に見えれば「アッシュグレー」、黄み・オレンジみのある暖色系に見えれば「ベージュ」。
4. 画像の照明・ホワイトバランスによる色かぶり（例：暖色照明で黒髪が茶色っぽく写る等）に惑わされず、髪全体のトーンで総合的に判断すること。
`.trim();
